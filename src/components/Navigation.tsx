  ...
  return (
    <nav className="sticky top-0">
      <Toolbar.Root className={`${styles.toolbar}`} aria-label="navigation">
        <Toolbar.Link className="text-3xl font-bold" href="/">
          Bookmark Organizer
        </Toolbar.Link>
          ...
          <AccessibleIcon.Root label="dashboard">
            <DashboardIcon
              className={`cursor-pointer ${
                page?.route?.isDashboardPage ? "text-tertiary-600" : ""
              }`}
              height={`${iconSize}px`}
              width={`${iconSize}px`}
            />
      ...
      <Toolbar.Link href="/account">
        <Label.Root htmlFor="account">
          <AccessibleIcon.Root label="account">
            <AvatarIcon
              className={`cursor-pointer ${
                page?.route?.isAccountPage ? "text-tertiary-600" : ""
              }`}
              height={`${iconSize}px`}
              width={`${iconSize}px`}
            />
      ...
      <Toolbar.Link href="/settings">
        <Label.Root htmlFor="settings">
          <AccessibleIcon.Root label="settings">
            <GearIcon
              className={`cursor-pointer ${
                page?.route?.isSettingsPage ? "text-tertiary-600" : ""
              }`}
              height={`${iconSize}px`}
              width={`${iconSize}px`}
            />
  ...
  return (
    <div className="flex gap-10">
      <div className={`${styles.toolbar__search_bar} flex items-center gap-1`}>
        <input
          type="search"
          className={`${styles.toolbar__search_bar__input} bg-white/10 hover:bg-white/20`}
          defaultValue=""
        />
        <Label.Root htmlFor="search">
          <AccessibleIcon.Root label="search">
            <MagnifyingGlassIcon
              className={`${styles.toolbar__search_bar__icon}`}
              height={`${iconSize}px`}
              width={`${iconSize}px`}
            />
          </AccessibleIcon.Root>
      ...
      <Toolbar.ToggleGroup
        className={`${styles.toolbar__actions}`}
        type="multiple"
      ...
      >
        <Tooltip.Provider>
          <Tooltip.Root>
            <Tooltip.Portal>
              <Tooltip.Content
                className={`${styles.toolbar__actions__tool_tip}`}
                sideOffset={5}
              >
                only not starred
                <Tooltip.Arrow
                  className={`${styles.toolbar__actions__tool_tip__icon}`}
                />
              </Tooltip.Content>
            </Tooltip.Portal>
